...
const Simplelogin = () => {
    const {googleLoginFunction,emailLoginfunction,formSubmitFunction2,error,emailfunction,passwordFunction,updateName, user}=UseAuth()
...
const reDireact_uri2=location.state?.from || '/home'

const hendleemialloginfunction=()=>{
  emailLoginfunction()
  .then((userCredential) => {
    history.push(reDireact_uri2)
  })
}
    ...
    return (
      ...
      <div class="right-content-form">
          <form onSubmit={(event)=>formSubmitFunction2(event)}>
            
      
      
      <div class="form-title">
      <div>
      <span>L</span><span>o</span><span>g</span><span>i</span><span>n</span><span></span><span>f</span><span>o</span><span>r</span><span>m</span>
      ...
      <Link to='/login'>haven't you account?</Link>
      
      </div>
      <button  className='mt-3 registerbtn' onClick={hendleemialloginfunction} >login</button>
